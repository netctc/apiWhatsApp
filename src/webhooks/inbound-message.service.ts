    ...
    try {
      const persisted = await this.prisma.$transaction(async (transaction) => {
        ...
        await transaction.contact.updateMany({
          where: {
            id: contact.id,
            OR: [{ lastInboundAt: null }, { lastInboundAt: { lt: inboundAt } }],
          },
          data: { lastInboundAt: inboundAt, serviceWindowExpiresAt },
        });
      ...
      if (persisted?.conversationId) {
        await this.realtime.publish(tenantId, "conversation.updated", {
          conversationId: persisted.conversationId,
        });
        await this.realtime.publish(tenantId, "conversation.message.created", {
          conversationId: persisted.conversationId,
